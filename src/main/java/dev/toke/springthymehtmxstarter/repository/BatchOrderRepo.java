...
public interface BatchOrderRepo extends JpaRepository<BatchOrder, Long> {
    ...
    @Query(value = "SELECT * FROM BatchOrder WHERE workplan_id = :workPlanId", nativeQuery = true)
    List<BatchOrder> findAllByWorkPlanId(Long workPlanId);
    @Query(value = "SELECT * FROM BatchOrder WHERE workplan_id IS NULL OR workplan_id = :batchId", nativeQuery = true)
